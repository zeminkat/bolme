(() => {
  ...
  // Floating controls (shown in fullscreen so navigation never disappears)
  ...
  // Viewport helpers (mobile address bar friendly)
  ...
  let coverMode = window.matchMedia('(max-width: 640px)').matches; // default: cover on phones
  let pseudoOn = false; // fallback fullscreen-like mode for mobile browsers without Fullscreen API
  const isOn = () => pseudoOn || isFullscreen();
  ...
  function resizeStage() {
    ...
    const isCover = coverMode;

    const headerVisible = !(document.body.classList.contains('uiHidden'));
    // In cover mode, the header is an overlay; don't reserve space for it.
    const headerH = (headerEl && headerVisible && !isCover) ? headerEl.getBoundingClientRect().height : 0;

    // footer shown only on larger screens
    const footerH = (footerEl && window.matchMedia('(min-width: 641px)').matches)
      ? footerEl.getBoundingClientRect().height
      : 0;

    // Available box for the stage
    const availW = vw;
    const availH = Math.max(120, vh - headerH - footerH);

    // On desktop in "fit" mode, keep it from being comically huge.
    const desktopCap = (!isCover && vw > 900) ? 1100 : Infinity;
    const maxW = Math.min(availW, desktopCap);
    const maxH = availH;

    let w, h;
    if (isCover) {
      // COVER: stage should cover the available box (may crop)
      w = maxW;
      h = w / STAGE_ASPECT;
      if (h < maxH) {
        h = maxH;
      ...
      // FIT: stage should fit inside the available box (may letterbox)
      w = maxW;
      h = w / STAGE_ASPECT;
      if (h > maxH) {
        h = maxH;
  ...
  // Keep sizing stable on mobile when the address bar shows/hides
  const vv = window.visualViewport;
  if (vv) vv.addEventListener('resize', resizeStage, { passive: true });

  window.addEventListener('resize', resizeStage, { passive: true });
  window.addEventListener('orientationchange', () => setTimeout(resizeStage, 60), { passive: true });
  ...
  function updateFsUi() {
    ...
    if (fExit) fExit.textContent = on ? '⤢' : '⛶';
  }

  function syncFsState() {
    const on = isOn();
    document.body.classList.toggle('fsOn', isFullscreen());
    document.body.classList.toggle('coverOn', pseudoOn);
    if (floatBar) floatBar.setAttribute('aria-hidden', on ? 'false' : 'true');
    updateFsUi();
  }


  document.addEventListener('fullscreenchange', () => { syncFsState(); resizeStage(); }, { passive: true });
  document.addEventListener('webkitfullscreenchange', () => { syncFsState(); resizeStage(); }, { passive: true });

  if (fsBtn) {
    fsBtn.addEventListener('click', async () => {
      // "Fullscreen" button: use real Fullscreen API when possible; otherwise fall back to pseudo fullscreen.
      if (!isOn()) {
        coverMode = true;
        pseudoOn = true;
        await requestFs(); // may fail on some mobile browsers (e.g., iOS Safari)
      } else {
        pseudoOn = false;
        await exitFs();
        // Restore default cover behavior based on device
        coverMode = window.matchMedia('(max-width: 640px)').matches;
      }
      syncFsState();
      resizeStage();
    });
  ...
  document.addEventListener('fullscreenchange', () => { updateFsUi(); resizeStage(); });
  document.addEventListener('webkitfullscreenchange', () => { updateFsUi(); resizeStage(); });

  // If user scrolls/zooms UI bars, re-fit quickly
  setVhVar();
  const STORAGE_KEY = "bolme_interaktif_v8";
  const slides = [];
  ...
  function markDraggables() {
    slides.forEach(slideDiv => {
      ...
      if (!svg) return;
      svg.querySelectorAll('g.shape').forEach(g => {
  ...
  function showSlide(n) {
    ...
    if (fPrev) fPrev.disabled = current === 1;
    if (fNext) fNext.disabled = current === slideCount;
    if (fInfo) fInfo.textContent = `${current}/${slideCount}`;
  ...
  if (fPrev) fPrev.addEventListener('click', () => showSlide(current - 1));
  if (fNext) fNext.addEventListener('click', () => showSlide(current + 1));
  if (fExit) fExit.addEventListener('click', async () => {
      pseudoOn = false;
      await exitFs();
      coverMode = window.matchMedia('(max-width: 640px)').matches;
      syncFsState();
