(() => {
  ...
  // Make the stage fit the viewport (especially on mobile/portrait)
  ...
  function resizeStage() {
    ...
    // stageWrap has padding (see CSS). Keep a small margin.
    const margin = 16;
    const maxW = Math.min(vw - margin, 1100);
    const maxH = vh - headerH - footerH - margin;
  ...
  window.addEventListener('orientationchange', () => setTimeout(resizeStage, 50), { passive: true });


  const STORAGE_KEY = "bolme_interaktif_v5";
  const slides = [];
